...
$texte_recherche = trim($_POST['recherche_membres_input']);
    if(isset($texte_recherche) && !empty($texte_recherche)){
        ...
        if ($tab == 0) {
            ...
            $html_div_recherche .=
            '
            <a href="redaction_message_direct.php?id_destinataire='.$x['id'].'&id='.$_SESSION['id'].'
            "><div style="width : 98%;padding:10px;border: 1px dotted rgba(105, 105, 105, 0.2);border-radius:5px;margin:5px;">
                <div style="width : 60%;display:inline-block;padding:10px;font-size:0.7rem;">'.
                $x['prenom'].' '.$x['nom'].'<br>'.
                'Ingénieur '.$x['ecole'].', promo '.$x['promotion'].'
                </div>
                <div style="width : 30%;display:inline-block;border-left: 1px dotted #cabcbc;">
                <p style="width:100%;height:100%;text-align: center;"><img src="../membres/avatar/'.$x['avatar'].'" alt="photo de profil" width=35px height=35px style="border-radius:50%"></p>
                </div>
            </div>
            </a>'
            ;
