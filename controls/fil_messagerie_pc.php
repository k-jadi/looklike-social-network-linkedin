...
<body style="overflow: hidden;">
            ...
            ?>
                <div style="position:relative;display:flex;flex-direction:column;width:100%;height:100vh;overflow:hidden;">
                    <div style="background-color:#fff;position:fixed;top:0px;height:55px;width:100%;display:flex;border-bottom:1px dotted #000;padding:5px;">
                        <div style="width: 18%;">
                        ...
                        <div  style="width: 14%;display:flex;align-items:center;justify-content:center;border-left:1px dotted #000;">
                            <a href="corps_messagerie_pc.php?id=<?PHP echo $_SESSION['id']; ?>" style="text-decoration: none;color:#000;" >
                                <i class="fa-solid fa-arrow-left" style="font-size: 1.1rem;"></i>
                            </a>
                        </div>
                    </div>
                    <div style="padding-bottom:10px;height:calc(100vh - 115px);overflow:hidden;overflow-y: scroll;position:fixed;top:55px;width:100%;display:flex;flex-direction:column;font-family:Arial, Helvetica, sans-serif;row-gap:4px;" id="liste_messages">
                        <?PHP
                            ...
                            while ($afficher_mes_messages = $mes_messages->fetch()) {
                                if (($afficher_mes_messages['id_expediteur'] == $get_id OR $afficher_mes_messages['id_expediteur'] == $get_id_correspondant) AND ($afficher_mes_messages['id_destinataire'] == $get_id OR $afficher_mes_messages['id_destinataire'] == $get_id_correspondant)) {
                                    ...
                                    $message_format_origine = nl2br($afficher_mes_messages['message']);
                                    ...
                                    <p style="padding-right:10px;padding-left:10px;padding-bottom:10px;font-size:0.7rem;font-family:arial;border-bottom:1px dotted rgba(66, 61, 61, 0.2);">'.$message_format_origine.'</p>
                                    ';
                                }
                            }
                        ?>
                    </div>
                    <div style="border-top:1px dotted #000;background-color:rgba(66, 61, 61, 0.1);height:60px;position:fixed;bottom:2px;width:100%;padding:5px;" id="redaction_message">
                        <form id="formulaire_redaction_message">
                            <div style="width: 80%;display:inline-block;vertical-align:middle;">
                                <textarea name="textarea_redaction_message" id="textarea_redaction_message" maxlength="350" placeholder="Rédigez un message ..." rows="2" style="max-height: 140px;;display: block; margin: 0; padding: 8px; box-sizing: border-box; resize: vertical;width: 100%; padding:5px; border: 1px dotted rgba(105, 105, 105, 0.2); border-radius:10px "></textarea>
                            </div>
                            <div  style="width: 18%;display:inline-block;vertical-align:middle;">
                                <input type="submit" name="publication_message" id="publication_message" value="Envoyer" style="width: 100%;padding:5px;cursor:pointer;border-radius:5px;border:none;background-color:blue;color:#fff;font-weight:bold;font-size:0.7rem;visibility:hidden;">
                            </div>
                        </form>
                    </div>
                </div>    
